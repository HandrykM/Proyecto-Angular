...
@Component({
  selector: 'app-trivia-media',
  templateUrl: './trivia-media.html',
  styleUrls: ['./trivia-media.css'],
  imports: [CommonModule],
  standalone: true
})
export class TriviaMediaComponent implements OnInit, OnDestroy {
  ...
  private construirUrlTrivia(): void {
    ...
    this.triviaUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
      `/assets/trivia-media/index.html?${params.toString()}`
    );
  }
  ...
  private procesarActualizacionTrivia(data: any): void {
    ...
    if (this.totalPreguntas > 0) {
      ...
      this.progresoGeneral = Math.round((this.totalPreguntas / 10) * 100);
    }
  }
  ...
  private generarRecomendaciones(): void {
    ...
    if (this.precision < 50) {
      this.recomendaciones.push({
        icono: 'fas fa-book',
        texto: 'Revisa los módulos intermedios sobre reúso del agua'
      });
    }
    ...
    if (this.dificultadActual === 'media' && this.precision >= 80) {
      this.recomendaciones.push({
        icono: 'fas fa-level-up-alt',
        texto: '¡Estás listo para el nivel avanzado!'
      });
    }
  ...
  finalizarActividad(): void {
    const progreso = {
      ...
      idActividad: 6, // ID de la trivia intermedia
      completada: true,
      progreso: 100,
    ...
    this.actividadesService.guardarProgreso(progreso).subscribe({
      next: () => {
        console.log('Progreso de trivia intermedia guardado exitosamente');
